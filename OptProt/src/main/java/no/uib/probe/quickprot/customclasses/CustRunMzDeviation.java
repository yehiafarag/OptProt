 ...
 */
package no.uib.probe.quickprot.customclasses;
...
import static com.compomics.util.parameters.peptide_shaker.ProjectType.peptide;
 ...
 * @author Marc Vaudel
 */
public class CustRunMzDeviation {
     ...
     */
    public CustRunMzDeviation(
            String spectrumFileNameWithoutExtension,
            List<SpectrumMatch> spectrumMatchList,
        ...
        for (SpectrumMatch spectrumMatch : spectrumMatchList) {
            ...
            if (cpt1 > rtBinSize) {
                ...
                for (Entry<Double, ArrayList<Double>> entry : mzToErrorMap.entrySet()) {
                    ...
                    for (double err : entry.getValue()) {
                        System.out.println("cpt2 " + cpt2 + "   " + cpt1);
                        if (cpt2 < cpt1 / 2) {
    ...
    private static double calculatPeptideMass(){
    double mass=0;
